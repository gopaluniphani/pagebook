...
public interface RequestRepository extends CrudRepository<Request,String> {
    ...
    @Transactional(readOnly = true)
    @Query(value = "select requestor_id from Friend where user_id=?1", nativeQuery = true)
    List<String> findRequestorId(String userId);

    @Transactional
    @Modifying
    @Query("delete from Request pr where pr.userId=:i")
    void deleteRequestById(@Param("i") String userId);
}
